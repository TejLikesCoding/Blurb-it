...
class GUI:
    ...
    '''
    def handle_loved_menu(self):
        #clear the existing menu items
        self.menu.delete(0, tk.END)

        #get the list of loved books from the database
        loved_books = self.db.get_loved_books()

        #check if there are loved books and add them to the menu
        if not loved_books:
            self.menu.add_command(label="No loved books", state=tk.DISABLED)
        else:
            #add each loved book to the menu
            for book_name in loved_books:
                self.menu.add_command(label=book_name, 
                                    command=lambda b=book_name: self.display_loved_book(b))

        #post the menu at the location of the lovedbooks_button
    ...
    def toggle_love_book(self):
        ...
        '''
            ...
            if book_data:
                _, _, _, _, _, _, _, _, _, _, _, loved = book_data
                loved_bool = loved == "True"  #convert string to boolean
                if loved_bool:
    ...
    def display_loved_book(self, book_name):
    ...
    '''
    def display_rec_books(self, genre):
        ...
        if not rec_books:
            ...
            for book_name in rec_books:
                self.rec_menu.add_command(
                    label=book_name,
                    command=lambda b=book_name: self.display_loved_book(b)  # b=book_name creates a new scope capturing the current value
                )

    '''

    def display_rec_books(self, genre, current_book_name):
        self.rec_menu.delete(0, tk.END)
        rec_books = self.db.get_books_by_genre(genre)
        if not rec_books:
            self.rec_menu.add_command(label="No similar books found", state=tk.DISABLED)
    ...
    def prepare_and_show_book(self, book_name):
    ...
    def display_rec_books(self, genre):
        ...
        if not rec_books:
            ...
            for book_name in rec_books:
                # Ensure that the command calls the correct function to display the book
                self.rec_menu.add_command(
                    label=book_name,
                    command=lambda b=book_name: self.display_loved_book(b)  # This needs to call the correct function
                )
...
class Database:
    ...
    def toggle_loved_status(self, isbn):
        conn = self._connect()
        cursor = conn.cursor()
    ...
    def get_loved_book_data_by_name(self, name):
        conn = self._connect()
        cursor = conn.cursor()
    ...
    def get_book_data_by_name(self, name):
        conn = self._connect()
        cursor = conn.cursor()
